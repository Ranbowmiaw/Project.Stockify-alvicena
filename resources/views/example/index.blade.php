  ...
  {{-- TABLE 2 --}}
      <div class="grid w-full grid-cols-4 gap-4 mt-4 xl:grid-cols-2 2xl:grid-cols-1">      
        ...
        <div class="p-8 bg-white border border-gray-200 rounded-lg shadow-sm dark:border-gray-700 sm:p-6 dark:bg-gray-800">
          <h3 class="flex items-center mb-4 text-lg font-semibold text-gray-900 dark:text-white">User activity</h3>
                     ...
                     @foreach($activityLogs as $index => $log)
                        <tr>
                          ...
                          <td class="p-4 text-sm font-normal text-gray-900 whitespace-nowrap dark:text-white">
                            ...
                            <div class="text-base font-semibold text-gray-900 dark:text-white">{{ $log->user->name ?? 'Unknown' }}</div>
                            <div class="text-sm font-normal text-gray-500 dark:text-gray-400">{{ $log->user->email ?? '-' }}</div>
                            </div>
                          </td>
                          <td class="p-4 text-sm font-normal text-gray-900 whitespace-nowrap dark:text-white">
                              {{ $log->role ?? '-' }}
                ...
                <tbody class="bg-white dark:bg-gray-800">
                  @foreach($lap2  as $key => $transaction)
                    <tr>
                      ...
                      <td class="p-4 text-sm font-normal text-gray-900 whitespace-nowrap dark:text-white">{{ $transaction->quantity }}</td>
                      <td class="p-4 text-sm font-normal text-gray-900 whitespace-nowrap dark:text-white">Rp {{ number_format($transaction->product->purchase_price, 0, ',', '.') }}
                      <td class="p-4 text-sm font-normal text-gray-900 whitespace-nowrap dark:text-white">Rp {{ number_format($transaction->product->selling_price, 0, ',', '.') }}
                      <td class="p-4 text-sm font-normal text-gray-900 whitespace-nowrap dark:text-white">{{ ucfirst($transaction->status) }}</td>
                    </tr>
                    @endforeach
                </tbody>
              </table>
